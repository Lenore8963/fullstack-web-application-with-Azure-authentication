// cross-origin resource sharing (CORS) is enabled for all domains and methods, using the Express.js cors middleware. This is insecure and only used for demonstration purposes here. In production, you should modify this as to allow only the domains that you designate.
// On the web API side, passport-azure-ad verifies the incoming access token's signature and validates it's payload against the issuer and audience claims (defined in BearerStrategy constructor) using the passport.authenticate() API. In the BearerStrategy callback, you can add further validation steps, such as checking if the caller's tenant is in the allowed tenants list via the tid claim (for multi-tenant applications), or if the caller's account is homed or guest via the acct optional claim. You can also check if the caller belongs to right roles or groups via the roles or groups claim, respectively. For more information, visit: https://docs.microsoft.com/azure/active-directory/develop/access-tokens#validate-the-user-has-permission-to-access-this-data
